...
32. **Cavalo de Acompanhamento Longitudinal** — 8 tabelas novas (ADDITIVE-ONLY) para os Cavalos 4 e 5 do Motor Clinico: acompanhamento_cavalo (checkins mensais, visitas, retornos, intercorrencias com classificacao Verde/Amarelo/Vermelho), exames_evolucao (valores de exames com classificacao por tercos: PREOCUPANTE/BAIXO/MEDIANO/OTIMO/ALERTA), feedback_formulas (efeito percebido MELHORA/SEM_EFEITO/PIORA/EFEITO_COLATERAL), dados_visita_clinica (antropometria completa: peso/altura/IMC, PA, FC, bioimpedancia toggle, adipometro toggle, adesao percebida), arquivos_exames (upload com tipo SANGUE/ULTRASSOM/RESSONANCIA/TOMOGRAFIA/DENSITOMETRIA/OUTRO, organizacao automatica Google Drive), formulas_master (catalogo de formulas com composicao, efeitos esperados/colaterais, contraindicacoes como arrays), cascata_validacao_config (toggles configuráveis por Dr. Caio: enfermeira03/consultor03/medico03/medico_senior, certificado A1), validacoes_cascata (fluxo ENFERMEIRA03→CONSULTOR03→MEDICO03→MEDICO_SENIOR com status PENDENTE/APROVADO/REJEITADO e assinatura A1). Seed padrao: cascata desligada, todas etapas habilitadas. Origem OPERACIONAL (equipe) vs AUTONOMA (paciente externo)
