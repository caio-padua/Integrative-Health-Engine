...
- **Dashboard Global (Ecossistema):** When `consultoria_master` + "Todas as Clínicas" selected → "Dashboard Global" title with "Visao consolidada do ecossistema". KPIs: Clínicas Ativas, Total Pacientes, Delegações Pendentes, Taxa Resolução Geral. Section "Clinicas do Ecossistema" shows only consultoria-linked clinics (filtered by `consultor_unidades`). Bar chart of delegations per clinic. API: `GET /api/dashboard/consultoria`.
- **Dashboard Local (Unidade):** When `consultoria_master` selects a specific clinic → "[Nome]" + "Dashboard Local da Unidade" subtitle. Clinic-specific data: Pacientes Ativos, Demandas Abertas, Taxa Resolução, Custo Demandas (R$). Sections: Distribuição de Planos (Diamante/Ouro/Prata/Cobre), Demandas por Complexidade, Delegações (grid 2x2), Filas Operacionais, Últimas Demandas. Colored with clinic brand color. "Visao Local" indicator. API: `GET /api/dashboard/dono-clinica/:unidadeId`.
