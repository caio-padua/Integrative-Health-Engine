 ...
 *
 * Fix Dr. Claude audit 22/abr/2026 (versão canônica · WAVE-5):
 ...
 *
 * Extra local: query string `?incluir_removidas=true` no GET lista permite
 * auditoria de soft-deleted (master-only no front).
 */
...
// ─────────────────────────────────────────────
// GET /substancias — exclui soft-deleted por padrão.
// ?incluir_removidas=true (auditoria) inclui as deletadas.
// ─────────────────────────────────────────────
router.get("/substancias", async (req, res) => {
  ...
  const result = conditions.length > 0
    ? await db.select().from(substanciasTable).where(sql`${sql.join(conditions, sql` AND `)}`)
