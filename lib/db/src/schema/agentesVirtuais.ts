...
export const agentesPersonalidadeTable = pgTable("agentes_personalidade", {
  ...
  acolhimento: integer("acolhimento").notNull().default(5),
  firmeza: integer("firmeza").notNull().default(5),
  didatismo: integer("didatismo").notNull().default(5),
  delicadeza: integer("delicadeza").notNull().default(5),
  sensibilidade: integer("sensibilidade").notNull().default(5),
  nivelProtocolar: integer("nivel_protocolar").notNull().default(5),
...
export const agentesIdentidadeTable = pgTable("agentes_identidade", {
  id: serial("id").primaryKey(),
  agenteClinicaId: integer("agente_clinica_id").references(() => agentesClinicaTable.id).notNull().unique(),
  nomeExibicao: varchar("nome_exibicao", { length: 200 }),
  identificadorSemantico: varchar("identificador_semantico", { length: 200 }),
  emailFuncionalVinculado: varchar("email_funcional_vinculado", { length: 200 }),
  corPrimariaHex: varchar("cor_primaria_hex", { length: 20 }),
  emojiPrincipal: varchar("emoji_principal", { length: 20 }),
  assinaturaPadrao: text("assinatura_padrao"),
  tratamentoPreferencial: varchar("tratamento_preferencial", { length: 100 }),
  observacoesIdentidade: text("observacoes_identidade"),
  criadoEm: timestamp("criado_em", { withTimezone: true }).notNull().defaultNow(),
  atualizadoEm: timestamp("atualizado_em", { withTimezone: true }).notNull().defaultNow().$onUpdate(() => new Date()),
});

export const agentesFrasesTable = pgTable("agentes_frases", {
  id: serial("id").primaryKey(),
  agenteClinicaId: integer("agente_clinica_id").references(() => agentesClinicaTable.id).notNull(),
  tipoFrase: varchar("tipo_frase", { length: 80 }).notNull(),
  texto: text("texto").notNull(),
  pesoPrioridade: integer("peso_prioridade").notNull().default(1),
  ativa: boolean("ativa").notNull().default(true),
...
export const agentesRegrasTable = pgTable("agentes_regras", {
  id: serial("id").primaryKey(),
  agenteClinicaId: integer("agente_clinica_id").references(() => agentesClinicaTable.id).notNull().unique(),
  podeResponderSozinho: boolean("pode_responder_sozinho").notNull().default(false),
  requerValidacaoHumana: boolean("requer_validacao_humana").notNull().default(true),
  podeEscalar: boolean("pode_escalar").notNull().default(true),
  destinoEscalonamento: varchar("destino_escalonamento", { length: 100 }),
  slaMinutos: integer("sla_minutos").notNull().default(120),
  nuncaResolveVermelhoSemHumano: boolean("nunca_resolve_vermelho_sem_humano").notNull().default(true),
  nuncaOrientaAjusteClinicoCritico: boolean("nunca_orienta_ajuste_clinico_critico").notNull().default(true),
  nuncaFechaOuvidoriaSensivel: boolean("nunca_fecha_ouvidoria_sensivel").notNull().default(true),
  criadoEm: timestamp("criado_em", { withTimezone: true }).notNull().defaultNow(),
  atualizadoEm: timestamp("atualizado_em", { withTimezone: true }).notNull().defaultNow().$onUpdate(() => new Date()),
});

export const agentesVersionamentoTable = pgTable("agentes_versionamento", {
  id: serial("id").primaryKey(),
  agenteClinicaId: integer("agente_clinica_id").references(() => agentesClinicaTable.id).notNull(),
  ...
  descricaoMudanca: text("descricao_mudanca"),
  snapshotJson: jsonb("snapshot_json").$type<Record<string, any>>(),
  criadoEm: timestamp("criado_em", { withTimezone: true }).notNull().defaultNow(),
});
...
export const insertIdentidadeSchema = createInsertSchema(agentesIdentidadeTable).omit({ id: true, criadoEm: true, atualizadoEm: true });
export const insertFraseSchema = createInsertSchema(agentesFrasesTable).omit({ id: true, criadoEm: true, atualizadoEm: true });
export const insertRegraSchema = createInsertSchema(agentesRegrasTable).omit({ id: true, criadoEm: true, atualizadoEm: true });
export const insertVersionamentoSchema = createInsertSchema(agentesVersionamentoTable).omit({ id: true, criadoEm: true });
...
export type AgentesIdentidade = typeof agentesIdentidadeTable.$inferSelect;
